...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    correct_logprobs = np.zeros(probs.shape)
    for i in range(N):
        if probs[i, y[i]] == 0:
            correct_logprobs[i] = 100
        else:
    ...
    grads['W2'] = np.dot(hidden_layer.T, dscores) + 2 * reg * self.params['W2']
    ...
    grads['W1'] = np.dot(X.T, dhidden) + 2 * reg * self.params['W1']
  ...
  def predict(self, X):
    ...
    np.maximum(hidden_layer, 0, hidden_layer)
